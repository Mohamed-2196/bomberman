...
setInterval(() => {
  ...
  const playersWithLivesZero = gameState.players.filter(
    player => player.lives === 0
  )
  if (
    playersWithLivesZero.length >= gameState.players.length - 1 &&
    gameState.gameStarted
  ) {
    gameState.gameStarted = false
    setTimeout(() => {
      console.log('here 1')
      console.log(playersWithLivesZero, 'wut?')
      console.log(gameState.players, 'pt2')
...
function handlePlayerDisconnect () {
  ...
  if (alivePlayers.length <= 1 && gameState.gameStarted) {
    ...
    setTimeout(() => {
      ...
      io.emit(
        'gameOver',
        alivePlayers.length === 1 ? alivePlayers[0].name : null
      )
    }, 2000)
    clearTimeout()
  } else {
...
function createWallsObject () {
  ...
  for (let i = 0; i < 289; i++) {
    ...
    if (
      ...
      if (Math.random() < 0.6) {
        wall.powerup = true
        wall.powerName = ['bombs', 'speed', 'flames'][
  ...
  function explodeDirection (dx, dy, config) {
    for (let i = 1; i <= config.range; i++) {
      ...
      if (wall.type === 'wall') {
        ...
        setTimeout(() => {
          if (Math.random() < 0.6) {
            wall.powerup = ['bombs', 'speed', 'flames'][
              Math.floor(Math.random() * 3)
            ]
          }
          wall.isBurned = false
